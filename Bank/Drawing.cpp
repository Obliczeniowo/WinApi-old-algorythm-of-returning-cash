...
void Drawing::drawPoint(HDC hdc,int x,int y,enum DrawingPointStyle dps){
	switch(dps){
		case DrawingPointStyle::box:
			{
				frameRectangle(hdc, x - 5, y - 5,(UINT) 10,(UINT) 10);
			}
			break;
		case DrawingPointStyle::circle:
			{
				frameCircle(hdc, x, y,(UINT) 5);
			}
			break;
		case DrawingPointStyle::cross:
			{
				line(hdc,x-5,y-5,x+5,y+5);
				line(hdc,x-5,y+5,x+5,y-5);
			}
			break;
		case DrawingPointStyle::dot:
			{
				fillCircle(hdc, x, y,(UINT) 5);
			}
			break;
		case DrawingPointStyle::pluss:
			{
				line(hdc,x,y-5,x,y+5);
				line(hdc,x-5,y,x+5,y);
			}
			break;
	}
}
...
//Funkcje z rodziny textOut rysuj¹ce tekst i liczby na kontekœcie urz¹dzenia
void Drawing::textOut(HDC &hdc,int x,int y,LPCSTR tekst,int dlTekstu){
	if(dlTekstu>0)
...
void Drawing::textOut(HDC &hdc,int x,int y,int liczba,int ilCyfr){
	char *p;
	if(ilCyfr<3)
		ilCyfr=3;
...
void Drawing::textOut(HDC &hdc,int x,int y,double liczba,int ilCyfr){
	char *p;
	if(ilCyfr<3)
		ilCyfr=3;
	p=new char[ilCyfr+10];
...
void Drawing::textOut(HDC &hdc,int x,int y,LPCSTR tekst,int dlTekstu,COLORREF kTekstu,int bkMode){
	SetTextColor(hdc,kTekstu);
	SetBkMode(hdc,bkMode);
	textOut(hdc,x,y,tekst,dlTekstu);
}

void Drawing::textOut(HDC &hdc,int x,int y,int liczba,int ilCyfr,COLORREF kTekstu,int bkMode){
	SetTextColor(hdc,kTekstu);
	SetBkMode(hdc,bkMode);
	textOut(hdc,x,y,liczba,ilCyfr);
}

void Drawing::textOut(HDC &hdc,int x,int y,double liczba,int ilCyfr,COLORREF kTekstu,int bkMode){
	SetTextColor(hdc,kTekstu);
	SetBkMode(hdc,bkMode);
	textOut(hdc,x,y,liczba,ilCyfr);
}

void Drawing::drawText(HDC &hdc,int liczba,int ilCyfr,int x,int y,UINT szer,UINT wys,UINT uFormat){
	char* p;
	if(ilCyfr<3)
		ilCyfr=3;
	p=new char[ilCyfr+10];
...
void Drawing::drawText(HDC &hdc,double liczba,int ilCyfr,int x,int y,UINT szer,UINT wys,UINT uFormat){
	char* p;
	if(ilCyfr<3)
		ilCyfr=3;
	p=new char[ilCyfr+10];
...
void Drawing::drawText(HDC &hdc,LPCSTR tekst,int dlTekstu,RECT *rect,UINT uFormat){
	DrawText(hdc,tekst,dlTekstu,rect,uFormat);
}

void Drawing::drawText(HDC &hdc,LPCSTR tekst,int dlTekstu,int x,int y,UINT szer,UINT wys,UINT uFormat){
